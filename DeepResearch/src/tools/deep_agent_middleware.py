...
def _state_from_context(
    ctx: RunContext[DeepAgentDeps] | DeepAgentDeps | DeepAgentState | Any,
) -> DeepAgentState:
    ...
    if hasattr(deps, "state"):
        return deps.state
    if isinstance(ctx, DeepAgentDeps):
        return ctx.state
    if isinstance(ctx, DeepAgentState):
        return ctx
    if hasattr(ctx, "state"):
        return ctx.state
    raise TypeError("DeepAgent middleware context must provide a DeepAgentState")
...
class BaseMiddleware:
    ...
    async def process(
        self,
        agent: Agent,
        ctx: RunContext[DeepAgentDeps] | DeepAgentDeps | DeepAgentState,
        **kwargs,
    ) -> MiddlewareResult:
...
class PromptCachingMiddleware(BaseMiddleware):
    ...
    def _generate_cache_key(
        self, ctx: RunContext[DeepAgentDeps] | DeepAgentDeps | DeepAgentState
    ) -> str:
